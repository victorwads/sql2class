...
function processSQL(SQL, jpa, package){
	...
	for(className in tables){
		var javaCode = processTable(package, tables[className], jpa);
		var javaDaoCode = createJavaDaoClass(package, tables[className]);
		self.postMessage({
			type: 'javaClass',
			html: createJavaClass(className, javaCode, 'model/'),
			daoHtml: createJavaClass(daoPrefix+className, javaDaoCode, 'dao/'),
			javaCode: javaCode,
...
function processTable(package, classInfo, jpa){
...
function createJavaDaoClass(package, classInfo){

	var className = daoPrefix+classInfo.javaName;
	var fields = classInfo.fields;
	...
	function getJDBCFieldCode(field){
		...
		if(field.reference){
			...
			return 'o.set' + field.funcName + '( new ' + field.javaType + '().set' + field.referenceFunc + '( rs.get' + field.referenceType.capitalize() + '(++i) ) );\n';
		}else
	...
	'\n'+
	'import '+package+'model.'+classInfo.javaName+';\n'+
	'\n'+
	'public class ' + className + ' extends Conexao {\n'+
	'\n'+
	'\tpublic boolean inserir(' + classInfo.javaName + ' o) {\n'+
	'\t\tboolean ok = false;\n'+
	'\t\ttry {\n'+
	'\t\t\tint i = 0;\n'+
	...
	if(primaryKey.length){
		javaClassCode +=
		'\n'+
		'\tpublic boolean atualizar(' + classInfo.javaName + ' o) {\n';
	...
	javaClassCode +=
	'\n'+
	'\tpublic boolean excluir(' + classInfo.javaName + ' o) {\n'+
	'\t\ttry {\n'+
	'\t\t\tint i = 0;\n'+
	...
	'\t}\n'+
	'\n'+
	'\tpublic ' + classInfo.javaName + '[] listar() {\n'+
	'\t\tArrayList<' + classInfo.javaName + '> objs = new ArrayList<>();\n'+
	'\t\t' + classInfo.javaName + '[] rt = null;\n'+
	'\t\ttry {\n'+
	'\t\t\tint i;\n'+
	'\t\t\t' + classInfo.javaName + ' o;\n'+
	'\t\t\tPreparedStatement ps = con.prepareStatement("SELECT ' + fieldsNamesAll.join(', ') + ' FROM ' + classInfo.sqlName + '");\n'+
	'\t\t\tResultSet rs = ps.executeQuery();\n'+
	'\t\t\twhile (rs.next()) {\n'+
	'\t\t\t\ti = 0;\n'+
	'\t\t\t\to = new ' + classInfo.javaName + '();\n';
	...
	'\t\t\t\tobjs.add(o);\n'+
	'\t\t\t}\n'+
	'\t\t\trt = new ' + classInfo.javaName + '[objs.size()];\n'+
	'\t\t\ti = 0;\n'+
	'\t\t\tfor (' + classInfo.javaName + ' r : objs) {\n'+
	'\t\t\t\trt[i++] = r;\n'+
	'\t\t\t}\n'+
	'\t\t} catch (Exception e) {\n'+
	'\t\t\te.printStackTrace();\n'+
...
self.addEventListener('message', function(event) {
	...
	if(message.type && message.type == "SQL" && message.sql !== undefined)
		processSQL(message.sql, message.jpa, message.package.charAt( message.package.length-1 )==='.'?message.package:message.package+'.');
